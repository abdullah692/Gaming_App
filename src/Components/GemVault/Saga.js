...
import { storeSaga } from "../reduxtoolkit/smartContract";
...
export default function Saga({ Saga ,val}) {
  const dispatch = useDispatch();
  const [sagaVal, setsagaVal] = useState("Generation 01");
  // console.log('Data Val a',data);
  const handleSelect = (e) => {
    let a;
    // console.log('Saga Values',e.target.value);

    a = e.target.value;
    dispatch(storeSaga({ saga: a.replace("Generation 0", "") }));

    setsagaVal(e.target.value);
  };
  console.log('Saga Value',sagaVal);
  return (
